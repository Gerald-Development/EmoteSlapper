...
public class BotMessageListener implements EventListener {

	private Connection dbConnection = null;
	private Configuration config = new Configuration();
	private static final Logger LOGGER = Logger.getLogger(BotMessageListener.class.getName());
	...
	public void onEvent(GenericEvent event) {
		if (event instanceof MessageReceivedEvent) {
			...
			DatabaseInterface.shutdownConnection(dbConnection);
		}
	}
	...
	private void checkMessageTokens(List<String> messageTokens) {
		...
		for (String emote : invalidEmotes) {
			...
			DatabaseInterface.executeDatabaseQuery(dbConnection, "DELETE FROM NitroliteEmote WHERE emoteid = '" + ID + "'");
		}
